...
另外，Redis允许你进行一些常用的操作，比如，“strlen <key>”可以用户获取值的的长度；“getrange <key> <start> <end>”将返回指定区域内的值；“append <key> <value>”将追加value到已有的键值上（如果该键不存在就创建一个）。动手尝试，一下是我所得到的结果。
